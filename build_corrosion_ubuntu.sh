...
build_architecture() {
    ...
    for dist in "${arr[@]}"; do
        FULL_VERSION="${CORROSION_VERSION}-${BUILD_VERSION}+${dist}_${build_arch}_ubu"
        ...
        if ! docker build . -f Dockerfile.ubu -t "uncloud-corrosion-ubuntu-$dist-$build_arch" \
        ...
        id="$(docker create "uncloud-corrosion-ubuntu-$dist-$build_arch")"
...
if [ "$ARCH" = "all" ]; then
    echo "🚀 Building corrosion $CORROSION_VERSION-$BUILD_VERSION for all Ubuntu distributions..."
    echo ""
